...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-vs",
        "--val_size",
        help="validation dataset size. defalut value is 16",
        type=int,
        default=16,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--ngf", help="number of gen filters of first Convolution", type=int, default=64
    )
    parser.add_argument(
        "--ndf", help="number of dis filters of first Convolution", type=int, default=64
    )
    parser.add_argument(
        "--norm_type",
    ...
    parser.add_argument(
        "--dataset",
        default="facade",
        choices=["facades", "edges2shoes", "edges2handbags"],
        help='what is datasets to use. default is "facades"',
    )
    parser.add_argument(
        "--use_leaky2dc",
        action="store_true",
    ...
    parser.add_argument(
        "--num_workers", type=int, default=4, help="num_worker for Dataloader"
    )
    parser.add_argument(
        "-g",
        "--gpu",
    ...
    train_data_dir = pathlib.Path(
        "../data/pix2pix/{}/train".format(opt.dataset)
    ).resolve()
    val_data_dir = pathlib.Path("../data/pix2pix/{}/val".format(opt.dataset)).resolve()
    # transform
    ...
    def make_optimizer(model, lr=0.0002, beta1=0.5):
        optimizer = optim.Adam(params=model.parameters(), lr=lr, betas=(beta1, 0.999))
        return optimizer
